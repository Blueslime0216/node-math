...
export class Controller {
    ...
    _isChangingVariable: boolean = false; // 변수 노드 값 조절 중인지 여부
    ...
    mousemove(e: MouseEvent) {
        ...
        // 호버된 노드 체크
        viewport.nodes.slice().reverse().some((node) => { // 가장 위에 있는 노드를 찾기 위해 reverse를 넣음
            // 소켓 호버 비우기
    ...
    mouseup(e: MouseEvent) {
        ...
        // [ 노드 드래그 선택 기능 ]
        if (e.button === 0) state.isDragSelecting_cancel = false; // 왼쪽 버튼이 떼지면 드래그 선택 취소 상태 해제
    ...
    holdStart(e: KeyboardEvent) {
    };



    holding(e: KeyboardEvent) {
    };
