...
export class Viewport {
    ...
    drawEffects() {
        ...
        if (userSetting.mouseZoomEffect.isOn && effectStateManager.mouseZoomSign.isOn === true) {
            zoomEffect(this._ctx, this.lineThickness, (new Date().getTime()), effectStateManager.mouseZoomSign);
        }
        // 키보드 확대/축소 중심 이펙트
        if (userSetting.keyboardZoomEffect.isOn && effectStateManager.keyboardZoomCenterSign.isOn === true) {
            zoomEffect(this._ctx, this.lineThickness, (new Date().getTime()), effectStateManager.keyboardZoomCenterSign);
        }
    }
    resize() {
