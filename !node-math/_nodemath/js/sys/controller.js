...
export class Controller {
    constructor() {
        ...
        this._isNodeDragging = false; // 노드 드래그 중인지 여부
        this._nodeDraggingCancel = false; // 노드 드래그 취소 여부
    ...
    keydown(e) {
        ...
        if ($keyboard.isKeyDown('NumpadAdd') || $keyboard.isKeyDown('NumpadSubtract')) {
            effectStateManager.keyboardZoomCenterSign = {
                isOn: true,
                startTime: (new Date().getTime()),
                isApply: zoomApply(viewport, ($keyboard.isKeyDown('NumpadAdd') ? 1.1 : 0.9), canvas.width / 2, canvas.height / 2),
