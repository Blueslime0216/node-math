...
class PieMenu {
    constructor() {
        ...
        this.subGroup = null;
        this.isOpen = false;
        this.targetPos = { x: 0, y: 0 }; // 캔버스 내부 기준 위치
        this.activeCategory = null;
        this.initDOM();
    }
    getSlicePath(innerR, outerR, startAngle, endAngle, gap = 2) {
        // 간격을 위해 각도 약간 축소
    ...
    getTextPos(innerR, outerR, startAngle, endAngle) {
    ...
    initDOM() {
        if (document.getElementById('pie-menu-container'))
            return;
        ...
        document.addEventListener('mousedown', (e) => {
            var _a;
            if (this.isOpen && e.target !== this.container && !((_a = this.container) === null || _a === void 0 ? void 0 : _a.contains(e.target))) {
                this.hide();
            }
        });
    }
    showSubMenu(catId, parentStart, parentEnd) {
        if (!this.subGroup)
            return;
    ...
    show(clientX, clientY, canvasOffsetX, canvasOffsetY) {
        if (!this.container)
            return;
        this.targetPos = {
        ...
        slices.forEach(s => s.style.stroke = '');
        setTimeout(() => {
            var _a;
            (_a = this.container) === null || _a === void 0 ? void 0 : _a.classList.add('show');
        }, 10);
        this.isOpen = true;
    }
    hide() {
