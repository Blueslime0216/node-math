export default class keyboardEventListener {
    ...
    constructor() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (!this._keymap.has(e.key)) {
                this._keymap.add(e.key);
            }
            if (!this._codemap.has(e.code)) {
                this._codemap.add(e.code);
            }
            // 커스텀 keydown 핸들러 호출
            this._keydown(e);
        });
    ...
    isDownKey(key) { return this._keymap.has(key); }
    // 특정 key code가 눌려있는지 확인 (Set에 code가 포함되어 있는지 확인)
    isDownCode(code) { return this._codemap.has(code); }
    // keydown 이벤트 핸들러를 설정하는 setter
    set keydown(callback) { this._keydown = callback; }
    // keyup 이벤트 핸들러를 설정하는 setter
    set keyup(callback) { this._keyup = callback; }
    // 이벤트 리스너를 삭제하는 메서드
    dispose() {
