...
export default class Node{
    ...
    isDirty: boolean = true; // 값이 변경되어 재계산이 필요한지 여부
    ...
    markDirty() {
        if (this.isDirty) return; // 이미 dirty면 하위도 dirty
        this.isDirty = true;
    ...
    createSocket(type:SocketType, direction:TSocketDirection, index:number){
        this.sockets[direction].push(new Socket(this, type, direction, index));
        this.sockets.all.push(this.sockets[direction][index]);
    }
    createSockets(type:SocketType, direction:TSocketDirection, count:number){
    ...
    draw(){
        ...
        if (this.type === 'variable' || this.type === 'value' || this.type === 'display') {
            ...
            if (this.type === 'value') ctx.fillStyle = 'hsl(10, 60%, 80%)';
            else if (this.type === 'display') ctx.fillStyle = 'hsl(280, 70%, 80%)';
            else ctx.fillStyle = 'hsl(150, 70%, 80%)';
